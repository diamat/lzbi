...
InBankModel.prototype.removeRS = function(arr, callback) {
	var thism = this;
	async.forEach(arr, function(i, callback){
					thism.client.hmget(thism.pMain(i), thism.kInRS, function(err, repl) {
							if (err) callback();
							else {
							if(repl[0]!= null){
								var rs = repl[0];
								console.log(thism.kInRS()+' pMain('+i+')');
								thism.client.hmset(thism.pMain(i), thism.kInRS(), rs, function(err, res) {
									if (err) {
												callback(null,null)
										} else {
											console.log('res - '+res);
											console.log('Сохранил рс - '+rs);
										}			
								});
							}
							callback();
...
InBankModel.prototype.findeRS = function(number, callback) {
	for(var i=0;i<number;i++){
		this.client.hmget(this.pMain(i), this.kInRS(), this.kInRS, function(err, repl) {
			if (err) {
					console.log('Ошибка!');
			} else {
						console.log('repl - '+repl[0]);
						console.log('repl2 - '+repl[1]);
				} 
		}.bind(this));
	}
	callback(null,null);
...
InBankModel.prototype.findByDate = function (date1, date2, callback) {
	var thism = this;
	var list = [];
