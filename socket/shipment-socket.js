...
exports.SocketAction = function (socket, client) {
		...
		socket.on('sum_prod', function (n_poddon, number) {
			number = my_sys.strSanitize(number);
			var data = {'n_poddon': n_poddon, 'number': number};
			console.log(data);
			validator.validForm(data, 5, function(err, result){
				if(err) socket.emit('err_sum', err);
				else {
						var sum =  (data.n_poddon*data.number).toFixed(2);
						socket.emit('end_sum', sum); 

					}
			});
		});
		
		socket.on('new_shipment', function (array_table) {
			var data = my_sys.returnObj(array_table);
			data.date = my_sys.dateNorm(data.date);
			validator.validForm(data, 6, function(err, result){
				if(err) socket.emit('err_save', err);
				else {
					socket.emit('start_save');
					var shipment = new Shipment(client);
					var model = shipment.create(data);	
					if(model != 'error'){
						model.save(function (err, res) {
							if(err) socket.emit('err_save', err);
							else {
					...
					} else socket.emit('err_save', 'Ошибка сохранения отгрузки');
				}
			});
		});
