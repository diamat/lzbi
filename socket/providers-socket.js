...
exports.SocketAction = function (socket, client) {
		...
		socket.on('new_provider', function (name_form, id, array_table) {
		...
		socket.on('prod_select', function (c_id, prod_id) {
			c_id = parseInt(c_id);
			prod_id = parseInt(prod_id);
			if(isNaN(c_id) != true && isNaN(prod_id) != true){
				var spravprod = new SpravProdModel(client);
				spravprod.findNumber(c_id, prod_id, function (err, res) {
						if(err) socket.emit('prod_res', 0);
								else {
									socket.emit('prod_res', res);
									}
						});
			} else socket.emit('err_save', 'c_id или prod_id не соответствуют типу');
		});
		
		socket.on('prod_save', function (c_id, prod_id, number) {
			number = my_sys.strSanitize(number);
			var data = {'c_id': c_id, 'prod_id': prod_id, 'number': number};
			validator.validForm(data, 4, function(err, result){
				if(err) socket.emit('err_save', err);
				else {
						var spravprod = new SpravProdModel(client);
						spravprod.saveNumber(data.c_id, data.prod_id, data.number, function (err, res) {
							socket.emit('start_save');
							if(err) socket.emit('err_save', ''+error);
							else {
									socket.emit('end_save_number', res); 
								}
						});
					}
			});
